...
public class Cosmetics extends GUI {
    ...
    public Cosmetics(AuroraMCServerPlayer player) {
        ...
        this.setItem(0, 4, new GUIItem(Material.SKULL_ITEM, String.format("&3&l%s's Cosmetics", player.getName()), 1, "", (short) 3, false, player.getName()));
        ...
        this.setItem(1, 7, new GUIItem(Material.SKULL_ITEM, "&f&lHats", 1, String.format(";&r&fYou have unlocked **%s**&f out of **%s**&f hats.;;&r&7These are special items that appear;&r&7on your head", cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.HAT).filter(cosmetic -> cosmetic.hasUnlocked(player)).count(), cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.HAT).filter(cosmetic -> cosmetic.hasUnlocked(player) || cosmetic.showIfNotUnlocked()).count()), (short) 3, player.getActiveCosmetics().containsKey(Cosmetic.CosmeticType.HAT) && player.getActiveCosmetics().get(Cosmetic.CosmeticType.HAT) != null, "AuroraMC"));

        GUIItem guiItem = new GUIItem(Material.BANNER, "&b&lBanners", 1, String.format(";&r&fYou have unlocked **%s** &fout of **%s** &fbanners.;;&r&7These are messages that appear in;&r&7chat when you kill a player in any game.", cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.BANNER).filter(cosmetic -> cosmetic.hasUnlocked(player)).count(), cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.BANNER).filter(cosmetic -> cosmetic.hasUnlocked(player) || cosmetic.showIfNotUnlocked()).count()), (short)0, player.getActiveCosmetics().containsKey(Cosmetic.CosmeticType.BANNER) && player.getActiveCosmetics().get(Cosmetic.CosmeticType.BANNER) != null);
