...
public class Reward {
    ...
    public void apply(AuroraMCPlayer player, boolean send) {
        player.getStats().addXp(this.xp, send);
        player.getBank().addCrowns(this.crowns, true, send);
        player.getBank().addTickets(this.tickets, true, send);
        for (Map.Entry<Integer, Integer> entry : achievements.entrySet()) {
            player.getStats().achievementGained(AuroraMCAPI.getAchievement(entry.getKey()), entry.getValue(), send);
        }
            ...
            if (!player.getUnlockedCosmetics().contains(cosmetic)) {
                ...
                if (send) {
                    if (!AuroraMCAPI.isTestServer()) {
                        ScheduleFactory.scheduleAsync(() -> AuroraMCAPI.getDbManager().addCosmetic(player.getUniqueId(), cosmetic));
                    }
    ...
    public void apply(int id, PlayerStatistics statistics) {
        ...
        for (int cosmeticId : this.cosmetics) {
            ...
            if (!cosmetics.contains(cosmetic)) {
                if (!AuroraMCAPI.isTestServer()) {
                    ScheduleFactory.scheduleAsync(() -> AuroraMCAPI.getDbManager().addCosmetic(uuid, cosmetic));
                }
            }
        }
    }
