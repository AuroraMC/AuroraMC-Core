    ...
    public static UUID logException(Throwable t, AuroraMCPlayer executor, String commandSyntax) {
        ...
        ScheduleFactory.scheduleAsync(() -> {
            try {
                ...
                webhook.addEmbed(new DiscordWebhook.EmbedObject().setTitle("AuroraMC Watchdog").setDescription(String.format("A new %s has been logged. View at https://PLACEHOLDER/watchdog/exceptions?uuid=%s", t.getClass().getSimpleName(), uuid)).setColor(new Color(170, 0, 0)));
                try {
                    webhook.execute();
                } catch (IOException e) {
                    e.printStackTrace();
