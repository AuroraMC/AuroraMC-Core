...
public class PlusColour extends GUI {
    ...
    public PlusColour(AuroraMCPlayer player) {
        ...
        for (int i = 0; i <= 8; i++) {
            if (i < 6) {
                this.setItem(i, 0, new GUIItem(Material.STAINED_GLASS_PANE, "&3&lYour Plus Colour", 1, "", (short) 7));
                this.setItem(i, 8, new GUIItem(Material.STAINED_GLASS_PANE, "&3&lYour Plus Colour", 1, "", (short) 7));
            }
            this.setItem(0, i, new GUIItem(Material.STAINED_GLASS_PANE, "&3&lYour Plus Colour", 1, "", (short) 7));
            this.setItem(5, i, new GUIItem(Material.STAINED_GLASS_PANE, "&3&lYour Plus Colour", 1, "", (short) 7));
        }
    ...
    public void onClick(int row, int column, ItemStack item, ClickType clickType) {
        if (item.getType() == Material.LEATHER_CHESTPLATE) {
            ...
            if (player.getActiveSubscription() != null) {
                player.getActiveSubscription().setColor(color.getChar());
                player.getPlayer().sendMessage(AuroraMCAPI.getFormatter().pluginMessage("Plus", String.format("Your Plus colour was set to **%s**.", ChatColor.stripColor(item.getItemMeta().getDisplayName()))));
            } else {
