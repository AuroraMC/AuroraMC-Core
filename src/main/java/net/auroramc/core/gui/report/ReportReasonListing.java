...
public class ReportReasonListing extends GUI {
    ...
    public ReportReasonListing(AuroraMCPlayer player, int id, String name, PlayerReport.ReportType type) {
        ...
        for (int i = 0;i < 10;i++) {
            ...
            this.setItem(row, column, new GUIItem(((type == PlayerReport.ReportType.MISC)?Material.SIGN:Material.BOOK_AND_QUILL), "&3&l" + reason.getName(), 1, String.format(";&rClick here to report this;&rplayer for **%s**", reason.getName())));
            column++;
            if (column == 7) {
                row++;
                column = 2;
