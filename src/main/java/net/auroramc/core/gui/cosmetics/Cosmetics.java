...
public class Cosmetics extends GUI {
    ...
    public Cosmetics(AuroraMCPlayer player) {
        ...
        this.setItem(3, 4, new GUIItem(Material.NAME_TAG, "&9&lJoin Messages", 1, String.format(";&rYou have unlocked **%s** out of **%s** join messages.;;&r&7These are messages that appear in;&r&7chat when you join a game server.;;&bPurchase a rank at store.auroramc.net to;&buse a custom join message.", cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.JOIN_MESSAGE).filter(cosmetic -> cosmetic.hasUnlocked(player)).count(), cosmetics.values().stream().filter(cosmetic -> cosmetic.getType() == Cosmetic.CosmeticType.JOIN_MESSAGE).filter(cosmetic -> cosmetic.hasUnlocked(player) || cosmetic.showIfNotUnlocked()).count())));
