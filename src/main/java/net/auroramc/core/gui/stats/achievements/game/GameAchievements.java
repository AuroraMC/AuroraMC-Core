...
public class GameAchievements extends GUI {
    ...
    public GameAchievements(AuroraMCPlayer player, String targetName, PlayerStatistics targetStatistics, ItemStack item) {
        ...
        this.setItem(2, 3, new GUIItem(Material.IRON_SWORD, "&c&lDuels", 1, String.format("&rAchieved: **%s**;&rTotal Achievements: **%s**", targetStatistics.getAchievementsGained().keySet().stream().filter(achievement -> achievement.getCategory() == Achievement.AchievementCategory.GAME && achievement.getGameId() == 4).count(), totalDuelsAchievements)));
