...
public class PaintballStatistics extends GUI {
    ...
    public PaintballStatistics(AuroraMCPlayer player, String targetName, PlayerStatistics targetStatistics, PlusSubscription subscription, int playerId) {
        ...
        double kdr = ((targetStatistics.getStatistic(1, "deaths") == 0)?targetStatistics.getStatistic(2, "kills"):(double)targetStatistics.getStatistic(2, "kills") / targetStatistics.getStatistic(2, "deaths"));
        double finalKdr = (Math.round(kdr * 100))/100.0;
        this.setItem(2, 4, new GUIItem(Material.SNOW_BALL, "&b&lGame Statistics", 1, ";&rGames Played: **" + targetStatistics.getStatistic(2, "gamesPlayed") + "**;&rWins: **" + targetStatistics.getStatistic(2, "gamesWon") + "**;&rLosses: **" + losses + "**;&rWin/Loss Ratio: **" + finalWlr + "**;;&rCrowns Earned: **" + String.format("%,d", targetStatistics.getStatistic(2, "crownsEarned"))  + "**;&rTickets Earned: **" + String.format("%,d", targetStatistics.getStatistic(2, "ticketsEarned")) + "**;&rExperience Earned: **" + String.format("%,d", targetStatistics.getStatistic(2, "xpEarned")) + "**;;&rKills: **" + targetStatistics.getStatistic(2, "kills") + "**;&rDeaths: **" + targetStatistics.getStatistic(2, "deaths") + "**;&rKill/Death Ratio: **" + finalKdr + "**"));
        PlayerKitLevel hotPotatoLevel = AuroraMCAPI.getDbManager().getKitLevel(playerId, 2, 0);
        this.setItem(2, 6, new GUIItem(Material.SNOW_BALL, "&b&lKit Statistics", 1, ";&3&lTribute;&rLevel: **" + hotPotatoLevel.getLevel() + "**;&rTotal XP: **" + String.format("%,d", hotPotatoLevel.getTotalXpEarned()) + "**;&rCurrent Upgrade: **" + hotPotatoLevel.getLatestUpgrade() + "**"));

        long totalGameAchievements = AuroraMCAPI.getAchievements().values().stream().filter(achievement -> achievement.getCategory() == Achievement.AchievementCategory.GAME && achievement.getGameId() == 2).filter((Achievement::isVisible)).count() + player.getStats().getAchievementsGained().keySet().stream().filter(achievement -> achievement.getCategory() == Achievement.AchievementCategory.GAME && achievement.getGameId() == 2 && !achievement.isVisible()).count();

        this.setItem(3, 4, new GUIItem(Material.BOOK, "&bTag Achievements", 1, String.format("&rAchieved: **%s**;&rTotal Achievements: **%s**;;&aClick to view more!", player.getStats().getAchievementsGained().keySet().stream().filter(achievement -> achievement.getCategory() == Achievement.AchievementCategory.GAME && achievement.getGameId() == 2).count(), totalGameAchievements)));
    }
