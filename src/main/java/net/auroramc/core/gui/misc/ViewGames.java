...
public class ViewGames extends GUI {
    ...
    public ViewGames(AuroraMCPlayer player, List<GameLog> logs, String name) {
        ...
        for (GameLog log : logs) {
            this.setItem(row, column, new GUIItem(log.getGame().getItem(), log.getGame().getName(), 1, ";&r&fGame UUID: **" + log.getUuid() + "**;&r&fGame Started: **" + new Date(log.getData().getLong("start")) + "**;&r&fGame Ended:**" + new Date(log.getData().getLong("end")) + "**;;&r&aClick to get the link to the game log!", log.getGame().getData()));
