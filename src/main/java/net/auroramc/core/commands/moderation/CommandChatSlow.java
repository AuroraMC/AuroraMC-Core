        ...
        if (args.size() == 1) {
            ...
            for (AuroraMCPlayer player2 : AuroraMCAPI.getPlayers()) {
                if (player2.hasPermission("moderation") || player2.hasPermission("social") ||  player2.hasPermission("debug.info")) {
                    player2.getPlayer().sendMessage(AuroraMCAPI.getFormatter().pluginMessage("ChatSlow", String.format("A chat slow of **%s** was enabled on this server. The goose has granted you immunity from it because your rank is very pog!", length.getFormatted())));
                } else {
