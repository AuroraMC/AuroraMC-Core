...
public class Flex extends KillMessage {
    ...
    public String onKill(AuroraMCPlayer receiver, AuroraMCPlayer killer, AuroraMCPlayer victim, Entity entity, KillReason reason, int gameId) {
        ...
        switch (reason) {
            ...
            case ENTITY: {
                return String.format("**%s** has died **#%s** times to entities, this time at the hands of **%s**.", victimName, victim.getStats().getStatistic(gameId, "deaths;" + reason.name()), WordUtils.capitalizeFully(WordUtils.capitalizeFully(entity.getType().name().replace("_", " "))));
            }
            case DROWNING: {
                if (killer != null) {
