...
public class MentorList extends GUI {
    ...
    public MentorList(AuroraMCServerPlayer player, List<Mentor> mentors) {
        ...
        this.setItem(2, 8, new GUIItem(Material.SKELETON_SKULL, "&3&lUnassigned Pending Punishments"));

        int column = 0;
        int row = 0;
        for (Mentor mentor : mentors) {
            this.setItem(row, column, new GUIItem(Material.PLAYER_HEAD, String.format("&3&l%s's Mentees", mentor.getName()), 1, String.format("&r&fLook at %s's mentees.", mentor.getName()), (short)0, false, mentor.getName()));
            column++;
            if (column == 9) {
                column = 0;
                row++;
