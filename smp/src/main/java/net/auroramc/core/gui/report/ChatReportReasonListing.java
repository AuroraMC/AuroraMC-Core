...
public class ChatReportReasonListing extends GUI {
    ...
    public ChatReportReasonListing(AuroraMCServerPlayer player, int id, String name, PlayerReport.ChatType type) {
        ...
        for (int i = 0;i < 10;i++) {
            ...
            this.setItem(row, column, new GUIItem(Material.WRITABLE_BOOK, "&3&l" + reason.getName(), 1, String.format(";&r&fClick here to report this;&r&fplayer for **%s**", reason.getName())));
            column++;
            if (column == 7) {
                row++;
                column = 2;
    ...
    public void onClick(int row, int column, ItemStack item, ClickType clickType) {
        ...
        if (item.getType() == Material.ARROW) {
            ...
            for (int i = 0;i < 10;i++) {
                ...
                PlayerReport.ReportReason reason = reportReasons.get(pi);
                this.updateItem(row, column, new GUIItem(Material.WRITABLE_BOOK, "&3&l" + reason.getName(), 1, String.format(";&r&fClick here to report this;&r&fplayer for **%s**", reason.getName())));
            }
        } else {
